...
namespace AasxPackageExplorer
{
    ...
    public partial class MainWindow : Window, IFlyoutProvider
    {
        ...
        private async void CommandBinding_GeneralDispatch(string cmd)
        {
            ...
            {
                CommandBinding_ReuseSubassembly();
            }

            if (cmd == "associatemodulewithsubassemblies")
            {
                CommandBinding_AssociateModuleWithSubassmblies();
            }

            if (cmd == "createwireharnessorder")
            {
                CommandBinding_CreateWireHarnessOrder();
            }
        }
        ...
        public void CommandBinding_ReuseSubassembly()
        {
            // trivial things
            if (!_packageCentral.MainStorable)
            {
            ...
            // check, if required plugin can be found
            var pluginName = "AasxPluginVec";
            var actionName = "reuse-subassembly";
            var pi = Plugins.FindPluginInstance(pluginName);
            if (pi == null || !pi.HasAction(actionName))
            {
                var res = MessageBoxFlyoutShow(
            ...
            {
                MessageBoxFlyoutShow(
                    "No valid selection for reusing a subassembly. Only entities may be selected!", "Reuse Subassembly",
                    AnyUiMessageBoxButton.OK, AnyUiMessageBoxImage.Error);
                return;
            }
            ...
            {
                MessageBoxFlyoutShow(
                    "No valid selection for reusing a subassembly. Only entities below the same parent element may be selected!", "Reuse Subassembly",
                    AnyUiMessageBoxButton.OK, AnyUiMessageBoxImage.Error);
                return;
            }
        ...
        public void CommandBinding_AssociateModuleWithSubassmblies()
        {
            // trivial things
            if (!_packageCentral.MainStorable)
            {
            ...
            // check, if required plugin can be found
            var pluginName = "AasxPluginVec";
            var actionName = "associate-subassemblies-with-module";
            var pi = Plugins.FindPluginInstance(pluginName);
            if (pi == null || !pi.HasAction(actionName))
            {
                var res = MessageBoxFlyoutShow(
            ...
            {
                MessageBoxFlyoutShow(
                    "No valid selection for associating a module with subassemblies. Only entities may be selected!", "Associate Module with Subassemblies",
                    AnyUiMessageBoxButton.OK, AnyUiMessageBoxImage.Error);
                return;
            }
        ...
        public void CommandBinding_CreateWireHarnessOrder()
        {
            // trivial things
            if (!_packageCentral.MainStorable)
            {
            ...
            {
                MessageBoxFlyoutShow(
                    "No valid selection for creating a wire harness order. Only entities may be selected!", "Derive Wire Harness Order",
                    AnyUiMessageBoxButton.OK, AnyUiMessageBoxImage.Error);
                return;
            }
